...
similarity of GO terms based on Resnik's, Lin's, Jiang and Conrath's,
...
genes = DO2ALLEG[[1]]
genes
x <- enrichDO(genes, pvalueCutoff=0.05, qvalueCutoff=0.05)
head(summary(x))
@
...
\begin{center}
<<BARPLOT, fig=T>>=
plot(x,showCategory=6, type="bar")
@
...
\Rfunction{plot}  function will use them to label the gene nodes.

In the following example, we use the artificial expression values to
illustrate the usage.
<<pseudo expression>>=
set.seed(123)
expr <- log(2+rnorm(length(genes)), base=2)
names(expr) <- genes
@

The plot was re-generate by using this pseudo expression values as follows:
\begin{figure}[h]
\begin{center}
<<CNETPLOT with logFC, fig=T>>=
plot(x,showCategory=5, logFC=expr, categorySize="geneNum",output="fixed")
@
\caption{\label{cnetplot} Category-Network Plot of Enrichment Result}
\end{center}
\end{figure}
