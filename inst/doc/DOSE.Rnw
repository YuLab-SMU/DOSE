...
<<simplot, fig=TRUE, height=6, width=6>>=
simplot(s, 
...
\begin{figure}[ht]
\centering
\includegraphics[width=.6\textwidth]{DOSE-simplot}
\caption{visualizing similarity matrix}
\label{simplot}
\end{figure}
...
\subsubsection{max}
The \Rfunction{max} method calculates the maximum semantic similarity score over all pairs of DO terms beween these two DO term sets.
$$
...
cnetplot(x, categorySize="pvalue", logFC=geneList)
@

\begin{figure}[ht]
\centering
\includegraphics[width=.9\textwidth]{DOSE-cnetplot}
\caption{cnetplot of DO enrichment result}
\label{cnetplot}
\end{figure}
...
<<clusterProfiler, fig=TRUE, height=5, width=5>>=
...
\begin{figure}[ht]
\centering
\includegraphics[width=.6\textwidth]{DOSE-clusterProfiler}
\caption{DOSE with clusterProfiler}
...
\subsection{gseaAnalyzer fuction}
In \Rpackage{DOSE}, we implemented GSEA algorithm proposed by Subramanian \cite{subramanian_gene_2005} in \Rfunction{gseaAnalyzer} function.
...
<<gseaplot, fig=TRUE, height=9, width=9>>=
topID <- res[1,1]
topID
