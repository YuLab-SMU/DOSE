...
We have developed an \textsf{R} package \Biocpkg{clusterProfiler} \cite{yu_clusterprofiler:_2012} for comparing biological themes among gene clusters. \Biocpkg{DOSE} works fine with \Biocpkg{clusterProfiler} and can compare biological themes at disease perspective.
